...
###############################################################################
# Simple deployment script for EC2 single-instance MVP setup
#
# Usage:
#   ./scripts/deploy-mvp.sh [OPTIONS]
#
# Options:
...
#
# Prerequisites:
#   - .env.mvp file configured
#   - Caddyfile updated with domain
#   - Docker and Docker Compose installed
#
# Related Docs:
#   - docs/deployment/MVP_DEPLOYMENT_GUIDE.md
###############################################################################
...
Examples:
  ./scripts/deploy-mvp.sh                    # Full deployment
  ./scripts/deploy-mvp.sh --no-pull          # Deploy without pulling
  ./scripts/deploy-mvp.sh --no-backup        # Deploy without backup
...
check_prerequisites() {
    ...
    if [ ! -f "$ENV_FILE" ]; then
        ...
        log_error "Copy .env.mvp.example to .env.mvp and configure it"
        exit 1
    fi
...
backup_current_state() {
    ...
    if [ -f "$ENV_FILE" ]; then
        cp "$ENV_FILE" "$BACKUP_DIR/.env.mvp.${TIMESTAMP}"
    ...
    if docker ps -q -f "name=mongo" &> /dev/null; then
        ...
        docker exec $(docker ps -qf "name=mongo") mongodump --out=/tmp/backup_${TIMESTAMP} --quiet || log_warning "MongoDB backup failed (container may not be running)"
...
main() {
    log_info "========================================="
    log_info "Starting MVP Deployment"
    ...
    log_success "========================================="
    log_success "MVP Deployment Completed Successfully!"
    log_success "========================================="
    log_info ""
    log_info "Next steps:"
    log_info "  1. Test your application: https://yourdomain.com"
    log_info "  2. Check logs: docker-compose -f $COMPOSE_FILE logs -f"
    log_info "  3. Monitor resources: docker stats"
    log_info "  4. Setup backups: Add cron job for daily backups"
    log_info ""
    log_info "Scaling guide: docs/deployment/SCALING_ROADMAP.md"
}
